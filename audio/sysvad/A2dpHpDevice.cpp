...
// when the target removes the A2DP Sideband interface.
//

#pragma code_seg("PAGE")
NTSTATUS
A2dpHpDevice::EvtA2dpHpTargetQueryRemove
( 
...
#pragma code_seg("PAGE")
VOID
A2dpHpDevice::EvtA2dpHpTargetRemoveCanceled
( 
    _In_    WDFIOTARGET     IoTarget
)
{
...
#pragma code_seg("PAGE")
VOID
A2dpHpDevice::EvtA2dpHpTargetRemoveComplete
( 
    _In_    WDFIOTARGET     IoTarget
)
{
...
)
{
    ...
    openParams.EvtIoTargetQueryRemove = EvtA2dpHpTargetQueryRemove;
    openParams.EvtIoTargetRemoveCanceled = EvtA2dpHpTargetRemoveCanceled;
    openParams.EvtIoTargetRemoveComplete = EvtA2dpHpTargetRemoveComplete;
    ...
    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR, ("%!FUNC!: Creating unqie filter names for the A2DP device failed, 0x%x", ntStatus)),
        Done);
...
(
    _Out_ PA2DPHPDEVICE_CODEC_CAPABILITIES pCodecCaps
)
{
    PAGED_CODE();
    DPF_ENTER(("[A2dpHpDevice::GetA2dpHpCodecCaps]"));

    ASSERT(pCodecCaps != NULL);

    NTSTATUS ntStatus = STATUS_SUCCESS;

    RtlZeroMemory(pCodecCaps, sizeof(A2DPHPDEVICE_CODEC_CAPABILITIES));

    A2DPHPDEVICE_CODEC_CAPABILITIES Capabilities = { 0 };
    ...
    rp.RequestedSiop.Size       = 0;// No extra parameter after header

    //
    // Get the A2DP Configured Codec.
    //
    ntStatus = SendIoCtrlSynchronously(
        NULL,
        IOCTL_SBAUD_GET_SIOP,
        sizeof(SIDEBANDAUDIO_SIOP_REQUEST_PARAM),
        sizeof(A2DPHPDEVICE_CODEC_CAPABILITIES),
        &rp,
        &Capabilities
        );
    ...
    // All done.
    //
    RtlCopyMemory(pCodecCaps, &Capabilities, sizeof(A2DPHPDEVICE_CODEC_CAPABILITIES));
    ntStatus = STATUS_SUCCESS;
    ...
    for (;;)
    {
        ...
        if (NT_SUCCESS(params.IoStatus.Status))
        {
            ...
            case IOCTL_SBAUD_GET_CONNECTION_STATUS_UPDATE:
            {
                ...
                //
                A2DPHPDEVICE_CODEC_CAPABILITIES codecCaps = { 0 };
                A2DPHPDEVICE_CODEC_CAPABILITIES oldCodecCaps = { 0 };
                if (reqCtx->Buffer.connectionStatus.Connected)
                {
                    ntStatus = This->GetA2dpHpCodecCaps(&codecCaps);
                    if (NT_SUCCESS(ntStatus))
                    {
                        ExAcquireFastMutex(&This->m_ConnectedCodecLock);
                        RtlCopyMemory(&oldCodecCaps, &This->m_ConnectedCodecCaps, sizeof(A2DPHPDEVICE_CODEC_CAPABILITIES));
                        RtlCopyMemory(&This->m_ConnectedCodecCaps, &codecCaps, sizeof(A2DPHPDEVICE_CODEC_CAPABILITIES));
                        ExReleaseFastMutex(&This->m_ConnectedCodecLock);
                        if (codecCaps.A2dpHwSpecificParam01 != oldCodecCaps.A2dpHwSpecificParam01 ||
                            codecCaps.A2dpHwSpecificParam02 != oldCodecCaps.A2dpHwSpecificParam02 ||
                            codecCaps.A2dpHwSpecificParam03 != oldCodecCaps.A2dpHwSpecificParam03)
                        {
                            // Notify audio miniport about this change.
                ...
                oldStatus = (BOOL)InterlockedExchange(&This->m_ConnectionStatusLong, (LONG)reqCtx->Buffer.connectionStatus.Connected);
...
//=============================================================================
A2DPHPDEVICE_CODEC_CAPABILITIES SysVadSupportedA2dpCodecCaps[] =
{
    {0, 1, 2},
    { 1, 2, 3 },
    { 3, 4, 5 }
...
--*/
{
    ...
    {
        SIDEBANDAUDIO_IO_PARAM_HEADER DeviceCapsHeader;
        A2DPHPDEVICE_CODEC_CAPABILITIES supportedCodecs[SIZEOF_ARRAY(SysVadSupportedA2dpCodecCaps)];
    }SidebandDeviceCapsInputParams, *PSidebandDeviceCapsInputParams;

    SidebandDeviceCapsInputParams deviceCapsParams = { 0 };
    deviceCapsParams.DeviceCapsHeader = { SIDEBANDAUDIO_PARAMS_SET_A2DP, SIDEBANDAUDIO_PARAM_A2DP_CODECS, sizeof(SysVadSupportedA2dpCodecCaps) };
    RtlCopyMemory(&deviceCapsParams.supportedCodecs, SysVadSupportedA2dpCodecCaps, sizeof(SysVadSupportedA2dpCodecCaps));

    //
    // Get the A2DP Bypass speaker volume.
    //
    ntStatus = SendIoCtrlSynchronously(
        NULL,
...
--*/
{
    ...
    *ppSupportedFormatsIntersection = NULL;
    ...
    // Allocate memory needed to hold the info.
    //
    formats  = (PSIDEBANDAUDIO_SUPPORTED_FORMATS) ExAllocatePoolWithTag(NonPagedPoolNx, length, A2DPSIDEBANDTEST_POOLTAG03);
    if (formats == NULL)
    {
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    *PropValues = NULL;
    ...
    // Allocate memory.
    //
    propValues  = (PKSPROPERTY_DESCRIPTION) ExAllocatePoolWithTag(NonPagedPoolNx, Length, A2DPSIDEBANDTEST_POOLTAG02);
    if (propValues == NULL)
    {
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
...
--*/
{
    ...
    *PropValues = NULL;
    ...
    // Allocate memory needed to hold the info.
    //
    propValues = (PKSPROPERTY_DESCRIPTION) ExAllocatePoolWithTag(NonPagedPoolNx, length, A2DPSIDEBANDTEST_POOLTAG06);
    if (propValues == NULL)
    {
        ntStatus = STATUS_INSUFFICIENT_RESOURCES;
    }
    ...
    //
    // Get the A2DP Headphone Mute Property Values.
    //
    WDF_REQUEST_REUSE_PARAMS_INIT(
        &reuseParams,   
        WDF_REQUEST_REUSE_NO_FLAGS,
...
--*/
{
    ...
    if (m_pSpeakerDescriptor)
    {
        ...
        ntStatus = GetA2dpHpEndpointFormatsIntersection(&speakerDeviceFormats, &m_pSpeakerSupportedFormatsIntersection);
        IF_FAILED_ACTION_JUMP(
            ntStatus,
            DPF(D_ERROR, ("%!FUNC!: GetA2dpHpEndpointFormats: failed for speaker, 0x%x", ntStatus)),
            Done);
