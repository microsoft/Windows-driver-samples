...
Abstract:
    ...
    Sample Soundwire Extension Unit Driver.
...
RECORDER_LOG g_SDCAVXuLog;

// When creating private drops, use set C_DEFINES=-DDROP_STAMP=nnnn to add the drop timestamp to the initial trace
#ifndef DROP_STAMP
#define DROP_STAMP 0
#endif

PAGED_CODE_SEG
void SdcaXu_DriverUnload (_In_ WDFDRIVER Driver)
{
    PAGED_CODE(); 
...
--*/
{
    ...
    WDF_DRIVER_CONFIG_INIT(&wdfCfg, SdcaXu_EvtBusDeviceAdd);
    wdfCfg.EvtDriverUnload = SdcaXu_DriverUnload;
    
    //
    // Add a driver context. (for illustration purposes only).
    //
    WDF_OBJECT_ATTRIBUTES attributes;
    WDF_OBJECT_ATTRIBUTES_INIT_CONTEXT_TYPE(&attributes, SDCAXU_DRIVER_CONTEXT);
    ...
    g_SDCAVXuLog = logHandle;

    DrvLogInfo(g_SDCAVXuLog, FLAG_INIT, "SdcaVXu Driver loaded, %lld", DROP_STAMP);
